...
export default function App() {
  ...
  function editExpense(i) {
    setExpense({ name: expenses[i].name, value: `₹${expenses[i].value}` })
    removeExpense(i)
  }
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-violet-100 via-purple-200 to-pink-200 py-10 px-4">
        ...
        <form className="animate-fadeIn max-w-4xl mx-auto bg-white/60 backdrop-blur-md rounded-2xl shadow-lg p-10 flex flex-col md:flex-row gap-8 justify-center ">
          {/* Salary Input */}
        ...
        {/* TABLE */}
        {expenses.length > 0 && (
          <div className="animate-fadeIn max-w-4xl mx-auto mt-10 bg-white/70 backdrop-blur-md rounded-2xl shadow-lg p-6">
                ...
                <tbody>
                  {expenses.map((item, index) => (
                    <tr key={index} className="hover:bg-indigo-50 transition-colors">
                      ...
                      <td className="px-4 py-3 border-b font-medium text-gray-800">{item.name}</td>
                      <td className="px-4 py-3 border-b text-red-600 font-semibold">
                        ₹{item.value.toLocaleString("en-IN")}
                      </td>
                      <td className="px-4 py-3 border-b space-x-2 text-center">
                        <button
                        ...
                        <button
                          ...
                          className="px-4 py-1 rounded-lg bg-red-500 hover:bg-red-600 text-white text-sm font-medium shadow-sm transition"
                        >
                          Remove
                        </button>
                      </td>
